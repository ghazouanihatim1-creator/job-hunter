...
def try_greenhouse(slug):
    # Vrai board = 200 + au moins une offre publiee
    ok, r = _ok(f"https://boards-api.greenhouse.io/v1/boards/{slug}/jobs")
    if ok:
        try:
            return len(r.json().get("jobs") or []) > 0
        except Exception:
            return False
    return False
...
def try_lever(slug):
    # Vrai compte Lever = 200 + liste d'offres non vide
...
def try_smartrecruiters(slug):
    # ATTENTION : SmartRecruiters renvoie 200 + content vide meme pour un slug
    # inexistant. On n'accepte donc QUE si des offres reelles sont presentes.
    ok, r = _ok(f"https://api.smartrecruiters.com/v1/companies/{slug}/postings?limit=1")
    if ok:
        try:
            data = r.json()
            return int(data.get("totalFound", 0)) > 0 and len(data.get("content") or []) > 0
        except Exception:
            return False
    return False
